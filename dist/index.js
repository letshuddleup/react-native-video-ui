...
import { TouchableWithoutFeedback, TouchableHighlight, PanResponder, StyleSheet, Animated, Easing, Image, Text, View, } from 'react-native';
...
export default class VideoPlayer extends React.Component {
    ...
    */
    ...
    hideControlAnimation() {
        Animated.parallel([
            ...
            Animated.timing(this.animations.bottomControl.marginBottom, { toValue: -100 }),
        ]).start();
    }
    /**
    ...
    formatTime(time = 0) {
        ...
        const timePlayed = Math.min(Math.max(time, 0), this.state.duration);
    ...
    */
    ...
    renderControl(children, callback, style = {}) {
        return (<TouchableHighlight underlayColor="transparent" activeOpacity={0.3} onPress={() => {
    ...
    renderTopControls() {
        ...
        <Image source={topVignette} style={[styles.controls.column, styles.controls.vignette,
        ]}>
     ...
     */
    renderBack() {
        return this.renderControl(<Image source={backIcon}/>, this.methods.onBack);
    }
    /**
     * Render the volume slider and attach the pan handlers
     */
    renderVolume() {
        return (<View style={styles.volume.container}>
    ...
    renderBottomControls() {
        ...
        <Image source={bottomVignette} style={[styles.controls.column, styles.controls.vignette,
        ]}>
          {this.renderSeekbar()}
     ...
     */
    renderSeekbar() {
        return (<View style={styles.seekbar.container}>
        <View style={styles.seekbar.track} onLayout={(event) => { this.player.seekWidth = event.nativeEvent.layout.width; }}>
          <View style={[
            styles.seekbar.fill,
            {
    ...
    renderTitle() {
        if (this.opts.title) {
            return (<View style={[styles.controls.control, styles.controls.title]}>
     ...
     */
    renderTimer() {
        return this.renderControl(<Text style={styles.controls.timerText}>
        {this.calculateTime()}
      </Text>, this.methods.toggleTimer, styles.controls.timer);
    }
    /**
     * Show loading icon
     */
    renderLoader() {
        if (this.state.loading) {
          ...
          <Animated.Image source={loaderIcon} style={[
                {
                    transform: [
                        {
     ...
     */
    render() {
        return (<TouchableWithoutFeedback onPress={this.events.onScreenTouch} style={[styles.player.container, this.styles.containerStyle]}>
        <View style={[styles.player.container, this.styles.containerStyle]}>
          <Video {...this.props} ref={(videoPlayer) => { this.player.ref = videoPlayer; }} resizeMode={this.state.resizeMode} volume={this.state.volume} paused={this.state.paused} muted={this.state.muted} rate={this.state.rate} onLoadStart={this.events.onLoadStart} onProgress={this.events.onProgress} onError={this.events.onError} onLoad={this.events.onLoad} onEnd={this.events.onEnd} style={[styles.player.video, this.styles.videoStyle]} source={this.props.source}/>
          {this.renderError()}
